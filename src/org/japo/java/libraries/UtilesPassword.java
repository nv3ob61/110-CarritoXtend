...
public class UtilesPassword {
  ...
  public static final char[] pidePass(String user, User u) {
    boolean passOk = false;
    char[] pass;
    char[] passComp;
    
    do {
      //pedimos dos veces el pass y comparamos.
      ...
      if (Arrays.toString(pass).equals(Arrays.toString(passComp))
              && !Arrays.toString(user.toCharArray()).
                      equals(Arrays.toString(pass))) {
        System.out.printf("%nPASSWORD VALIDADO CON ÉXITO%n%n");
        passOk = true;
      } else if (Arrays.toString(
              u.getUser().toCharArray()).equals(Arrays.toString(pass))) {
